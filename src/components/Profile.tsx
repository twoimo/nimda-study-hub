    ...
    workExperience: [
      {
        ...
        endDate: "Present",
        description:
          "Lead penetration testing team, conduct security audits, and develop security policies.",
      },
      {
        ...
        endDate: "2021-02",
        description:
          "Implemented and maintained firewall systems, conducted vulnerability assessments, and responded to security incidents.",
      },
    ],
    education: [
      {
          ...
          <div id="resume-content">
            {isResumeView ? (
              <div className="space-y-6">
                <section>
                  <h4 className="text-lg font-semibold text-white mb-2 print:text-black">
                    Professional Summary
                  </h4>
                  <p className="text-gray-300 text-sm md:text-base print:text-gray-700">
                    {profile.bio}
                  </p>
                </section>
                <section>
                  ...
                  {profile.workExperience.map((exp, index) => (
                    <div key={index} className="mb-4">
                      <h5 className="text-green-400 font-semibold print:text-green-700">
                        {exp.position}
                      </h5>
                      <p className="text-gray-400 print:text-gray-600">
                        {exp.company} | {exp.startDate} - {exp.endDate}
                      </p>
                      <p className="text-gray-300 text-sm print:text-gray-700">
                        {exp.description}
                      </p>
                    </div>
                  ))}
                ...
                <section>
                  ...
                  <div className="flex flex-wrap">
                    {profile.skills.map((skill, index) => (
                      <span
                        key={index}
                        className="bg-gray-700 text-green-400 rounded-full px-3 py-1 text-sm font-semibold mr-2 mb-2 print:bg-gray-200 print:text-green-700"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="bg-green-600 hover:bg-green-700 text-white font-bold py-1 px-2 md:py-2 md:px-4 rounded text-sm"
                >
                  Save Changes
                </button>
              </form>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap">
                    {profile.skills.map((skill, index) => (
                      <span
                        key={index}
                        className="bg-gray-700 text-green-400 rounded-full px-2 py-1 text-xs md:text-sm font-semibold mr-2 mb-2"
                      >
                        {skill}
                      </span>
                    ))}
