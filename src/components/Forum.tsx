    ...
    <div className="cyberpunk-card p-4 rounded-lg">
      ...
      {sortedPosts.map(post => (
        <div key={post.id} className="mb-6 p-4 bg-gray-800 rounded">
          ...
          <p className="text-xs md:text-sm mb-2">Posted by: {post.author} | {post.createdAt.toLocaleString()}</p>
          ...
          {post.replies.map((reply) => (
            <div key={reply.id} className="ml-4 mb-2 p-2 bg-gray-700 rounded">
              <p className="text-xs mb-1">Reply by: {reply.author} | {reply.createdAt.toLocaleString()}</p>
