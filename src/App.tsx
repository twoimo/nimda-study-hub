  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-900 p-8 rounded-lg neon-border w-full max-w-md">
      ...
      <div className="min-h-screen bg-black text-green-500 p-4">
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center">
            ...
            <nav className={`${isMenuOpen ? 'flex' : 'hidden'} md:flex flex-col md:flex-row absolute md:relative top-16 right-4 md:top-auto md:right-auto bg-gray-900 md:bg-transparent p-4 md:p-0 rounded-lg md:rounded-none shadow-lg md:shadow-none z-50`}>
              <Link to="/" className="mb-2 md:mb-0 md:mr-4 hover:text-neon-blue transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>Dashboard</Link>
              <Link to="/forum" className="mb-2 md:mb-0 md:mr-4 hover:text-neon-blue transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>Forum</Link>
              <Link to="/tools" className="mb-2 md:mb-0 md:mr-4 hover:text-neon-blue transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>Tools</Link>
              <Link to="/categories" className="mb-2 md:mb-0 md:mr-4 hover:text-neon-blue transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>Categories</Link>
              <Link to="/profile" className="mb-2 md:mb-0 md:mr-4 hover:text-neon-blue transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>Profile</Link>
            </nav>
            <button
